...
class PulseEmitter : public File, public iface::Node {
 public:
  ...
  static std::unique_ptr<File> Deserialize(
      const msgpack::object& obj, const std::shared_ptr<Env>& env) {
    return std::make_unique<PulseEmitter>(env, msgpack::as_if<size_t>(obj, 0));
  }
  void Serialize(Packer& pk) const noexcept override {
    pk.pack(data_->n);
  }
  std::unique_ptr<File> Clone(const std::shared_ptr<Env>& env) const noexcept override {
    return std::make_unique<PulseEmitter>(env, data_->n);
  }
 ...
 private:
  ...
  class SatisfySock final : public InSock {
   public:
    SatisfySock(PulseEmitter* o, size_t idx) noexcept :
